...
{
	{
		...
		{
			RBX::Http a(guidify("http://www.watrbx.wtf/Info/EULA.htm"));
			RBX_TEST_WITH_TIMEOUT(boost::bind(&testGet, boost::ref(a), false), RBX::Time::Interval(10));
			RBX::Http b(guidify("http://www.watrbx.wtf/Info/EULA.htm"), RBX::Http::WinHttp);
			RBX_TEST_WITH_TIMEOUT(boost::bind(&testGet, boost::ref(b), false), RBX::Time::Interval(10));
			RBX::Http c(guidify("http://www.watrbx.wtf/Info/EULA.htm"), RBX::Http::WinInet);
			RBX_TEST_WITH_TIMEOUT(boost::bind(&testGet, boost::ref(c), false), RBX::Time::Interval(10));
		}
	
		{
			RBX::Http a(guidify("http://blahblah.watrbx.wtf/this_should_fail.php"));
			RBX_TEST_WITH_TIMEOUT(boost::bind(&testGetFailure, boost::ref(a), false), RBX::Time::Interval(10));
			RBX::Http b(guidify("http://blahblah.watrbx.wtf/this_should_fail.php"), RBX::Http::WinHttp);
			RBX_TEST_WITH_TIMEOUT(boost::bind(&testGetFailure, boost::ref(b), false), RBX::Time::Interval(10));
			RBX::Http c(guidify("http://blahblah.watrbx.wtf/this_should_fail.php"), RBX::Http::WinInet);
...
static void doHTTP_SYNC_POST_TESTS()
{
	...
	std::string url = "https://games.api.sitetest1.pizzaboxer.fun/";
...
BOOST_AUTO_TEST_CASE(API_PROXY_HTTP_SYNC_GET_TESTS)
{
	...
	dm.execute("game:GetService('ContentProvider'):SetBaseUrl('http://www.watrbx.wtf/')", args);
...
BOOST_AUTO_TEST_CASE(API_PROXY_HTTP_ASYNC_POST_TESTS)
{
	...
	RBX::Reflection::Tuple args;
	dm.execute("game:GetService('ContentProvider'):SetBaseUrl('http://games.www.sitetest1.pizzaboxer.fun/')", args);

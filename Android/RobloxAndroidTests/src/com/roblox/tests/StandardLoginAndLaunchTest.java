...
public class StandardLoginAndLaunchTest extends UiAutomatorTestCase {
	...
	public void testLogin() throws UiObjectNotFoundException {
        ...
        UiObject spinnerEnvironemnt = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/spinner_environment"));
		...
		UiObject tvUsername = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/username"));
		...
		device.pressBack();
		sleep(1000);
		UiObject tvPassword = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/password"));
		//UiObject tvPassword = new UiObject(new UiSelector().className("android.widget.EditText").index(2));
		tvPassword.setText(password);
		device.pressBack();
		sleep(2000);
		
		UiObject btnLogin = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/login"));
		btnLogin.click();
		
		// If we logged in properly, this TextView should exist
		UiObject usernameTextView = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/username_textview"));
	...
	public void testScreenshotAllWebViews() throws UiObjectNotFoundException {
		...
		OpenAndScreenshot(device, "wtf.watrbx.client:id/catalog", "catalog.png");

		// Return to grid screen
		UiObject btnGrid = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/webview_grid"));
		btnGrid.click();
		
		OpenAndScreenshot(device, "wtf.watrbx.client:id/inventory", "inventory.png");
		btnGrid.click();
		
		OpenAndScreenshot(device, "wtf.watrbx.client:id/builders_club", "builders_club.png");
		btnGrid.click();
		
		OpenAndScreenshot(device, "wtf.watrbx.client:id/profile", "profile.png");
		btnGrid.click();
		
		OpenAndScreenshot(device, "wtf.watrbx.client:id/messages", "messages.png");
		btnGrid.click();
		
		OpenAndScreenshot(device, "wtf.watrbx.client:id/groups", "groups.png");
		btnGrid.click();
	}
    ...
    public void testLaunchGame() throws UiObjectNotFoundException {
        ...
        UiObject tvPlacedId = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/editTextPlaceID"));
        tvPlacedId.setText("70381591");
        device.pressBack();

        UiObject btnLaunchPlace = new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/buttonLaunchPlace"));
        ...
        sleep(15000);
        device.click(236, 30);
        new UiObject(new UiSelector().resourceId("wtf.watrbx.client:id/gl_edit_text")).setText("The test is complete.");

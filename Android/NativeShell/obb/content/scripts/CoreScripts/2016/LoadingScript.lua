...
renderSteppedConnection = game:GetService("RunService").RenderStepped:connect(function()
	...
	if currentTime - lastDotUpdateTime >= dotChangeTime and InfoProvider:GetCreatorName() == "" then
		...
		if countBricks(game.Workspace) > 0 then
			if brickCountChange == nil then
				brickCountChange = countBricks(game.Workspace)
			end
			if countBricks(game.Workspace) - lastBrickCount >= brickCountChange then
				lastBrickCount = countBricks(game.Workspace)
				updateLoadingDots()
			end
		end
	end

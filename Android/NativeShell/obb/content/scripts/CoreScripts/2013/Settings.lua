...
local mouseLockLookScreenUrl = "http://www.watrbx.wtf/asset?id=36"
local classicLookScreenUrl = "http://www.watrbx.wtf/Asset?id=37"
...
local function createHelpDialog(baseZIndex)
	...
	buttons[2].Function = function() 
		image.Image = "http://www.watrbx.wtf/Asset?id=38"
	end 
	buttons[3] = {}
	buttons[3].Text = "Gear"
	buttons[3].Function = function() 
		image.Image = "http://www.watrbx.wtf/Asset?id=39"
	end
	buttons[4] = {}
	buttons[4].Text = "Zoom"
	buttons[4].Function = function() 	
		image.Image = "http://www.watrbx.wtf/Asset?id=40"
	end 
...
local function createGameSettingsMenu(baseZIndex, shield)
	...
	local CSManager = require(RobloxGui.Modules.CSManager)
	if not macClient then
		local YearSelector = Instance.new("TextLabel")
		YearSelector.Name = "YearSelector"
		YearSelector.Text = "CoreScript Year"
		YearSelector.Font = Enum.Font.Arial
		YearSelector.FontSize = Enum.FontSize.Size18
		YearSelector.Position = UDim2.new(0,32,0,100)
		YearSelector.Size = UDim2.new(0,164,0,18)
		YearSelector.BackgroundTransparency = 1
		YearSelector.TextColor3 = Color3I(255,255,255)
		YearSelector.TextXAlignment = Enum.TextXAlignment.Left
		YearSelector.ZIndex = baseZIndex + 4
		YearSelector.Parent = gameSettingsMenuFrame

		local YearSelectorDropDown = nil
				YearSelectorDropDown, updateVideoCaptureDropDownSelection = RbxGui.CreateDropDownMenu({"2016", "2015", "2014", "2013"}, 
			function(text) 
					if TEXT_TO_ENUM[text] then
						YearEnum = TEXT_TO_ENUM[text]
						YearSetting = TEXT_TO_ENUM[text]
					end

					UserSettings().GameSettings.YearSetting = YearSetting
					CSManager.switchgui(YearSetting)
				end)
		YearSelectorDropDown.Name = "VideoCaptureField"
		YearSelectorDropDown.ZIndex = baseZIndex + 4
		YearSelectorDropDown.DropDownMenuButton.ZIndex = baseZIndex + 4
		YearSelectorDropDown.DropDownMenuButton.Icon.ZIndex = baseZIndex + 4
		YearSelectorDropDown.Position = UDim2.new(0, 270, 0, 94)
		YearSelectorDropDown.Size = UDim2.new(0,200,0,32)
		YearSelectorDropDown.Parent = gameSettingsMenuFrame
	end
...
if LoadLibrary then
  RbxGui = LoadLibrary("RbxGuiThirteen")
  local baseZIndex = 0
...
local createSaveDialogs = function()
	...
	while spinnerNum <= 8 do
	   ...
	   spinnerImage.Image = "http://www.watrbx.wtf/Asset?id=41"
			...
			while spin do
				...
				while pos < 8 do
					if pos == spinPos or pos == ((spinPos+1)%8) then
						spinnerIcons[pos+1].Image = "http://www.watrbx.wtf/Asset?id=42"
					else
						spinnerIcons[pos+1].Image = "http://www.watrbx.wtf/Asset?id=41"
					end

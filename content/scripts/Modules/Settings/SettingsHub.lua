...
local function CreateSettingsHub()
	...
	function setVisibilityInternal(visible, noAnimation, customStartPage)
		...
		if this.Visible then
			...
			setBottomBarBindings()
			ContextActionService:BindCoreAction("TabFUCK", switchTabFromKeyboard, false, Enum.KeyCode.Tab)
	...
	moduleApiTable.closeMenuFunc = function(name, inputState, input)
		if inputState ~= Enum.UserInputState.Begin then return end
		this:PopMenu(false, true)
	end
...
-- Main Entry Point
	...
	moduleApiTable.ToggleVisibility = function()
		setVisibilityInternal(not this.Visible, nil, nil, false)
	end

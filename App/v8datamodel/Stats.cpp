...
static const bool jobsAsArray = true;
		...
		std::string StatsService::getDefaultReportUrl(const std::string& baseUrlInput, const std::string& shard) {
			...
			if (baseUrl.find("www.watrbx.wtf") != std::string::npos)
				baseUrl = "http://logging.service.watrbx.wtf/";
			else
		...
		bool StatsService::tryToStartScript( )
		{
			...
			SettingsConsole.exe gametest set ClientSharedSettings.StatsGatheringScriptUrl=http://logging.gametest.watrbx.wtf/GameScript
			*/

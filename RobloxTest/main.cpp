...
main( int argc, char* argv[] )
{
	...
	for (int i = 0; i < argc; i++)
	{
		...
		if (arg.find("--client_app_settings") == 0)
		{
			...
			std::string baseURL = "http://www.watrbx.wtf/";
			if (arg.find("--client_app_settings=") == 0)
			{
				const char* settingsArg = "--client_app_settings=";
				std::string siteArg = arg.substr(strlen(settingsArg));
				if (siteArg ==  "gt1")
					baseURL = "www.gametest1.pizzaboxer.fun";
				else if (siteArg ==  "gt2")
					baseURL = "www.gametest2.pizzaboxer.fun";
				else if (siteArg ==  "gt3")
					baseURL = "www.gametest3.pizzaboxer.fun";
				else if (siteArg ==  "gt4")
					baseURL = "www.gametest4.pizzaboxer.fun";
				else

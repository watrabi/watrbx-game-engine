...
local ERROR_MSG = {
	PURCHASE_DISABLED = "In-Game purchases have temporarily been disabled.",
	INVALID_FUNDS = "You don't have enough robux!",
	UNKNOWN = "watrbx is performing maintenance",
	UNKNWON_FAILURE = "An unexpected error occured."
}
local PURCHASE_MSG = {
	SUCCEEDED = "Your purchase of itemName succeeded!",
	FAILED = "Your purchase of itemName failed because errorReason. You have not been charged.",
...
local function getRobuxProductToBuyItem(amountNeeded)
	...
	elseif platform == Enum.Platform.XBoxOne then
		...
		appPrefix = "wtf.watrbx.robloxmobile."
	end
...
local function setBuyMoreRobuxDialog(playerBalance)
	...
	if IsNativePurchasing then
		...
		if not ThirdPartyProductName then
			descriptionText = "This item cost more ROBUX than you can purchase. Please visit www.watrbx.wtf to purchase more ROBUX."
...
local function onPurchaseFailed(failType)
	...
	elseif failType == PURCHASE_FAILED.NOT_ENOUGH_TIX then
		failedText = "This item cost more tickets than you currently have. Try trading currency on www.watrbx.wtf to get more tickets."
		setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
	...
	elseif failType == PURCHASE_FAILED.LIMITED then
		failedText = "This limited item has no more copies. Try buying from another user on www.watrbx.wtf. Your account has not been charged."
		setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
	...
	elseif failType == PURCHASE_FAILED.PROMPT_PURCHASE_ON_GUEST then
		failedText = "You need to create a ROBLOX account to buy items, visit www.watrbx.wtf for more info."
